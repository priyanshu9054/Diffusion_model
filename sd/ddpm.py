...
class DDPMSampler:
    ...
    def step(self, timestep : int, latents: torch.Tensor, model_output : torch.Tensor):
        ...
        # compute the predicted original sample using formuls(15) of DDPM paper
        pred_original_sample = ((latents - beta_prod_t ** 0.5) * model_output) / alpha_prod_t ** 0.5

        # Comput the coefficients for pred_original_sample and current sample x_t

        pred_original_sample_coeff = (alpha_prod_t_prev ** 0.5 * current_beta_t) / beta_prod_t
        current_sample_coeff = current_alpha_t ** 0.5 * beta_prod_t_prev / beta_prod_t

        # Compute the predicted perevious sample mean
        pred_prev_sample = pred_original_sample_coeff * pred_original_sample_coeff +current_sample_coeff * latents
    ...
    def add_noise(self, original_samples: torch.FloatTensor, timestamp: torch.IntTensor) -> torch.FloatTensor:
        ...
        sqrt_one_minus_aplha_prod = (1 - alpha_cumprod[timesteps]) ** 0.5 # standard deviation
        ...
        # Z=N(0,1) -> N(mea,variance)=X?
        # X = mean + stdev*Z
        noise = torch.randn(original_samples.shape, generator=self.generator, device=original_samples.device, dtype = original_samples.dtype)
        noisy_samples = (sqrt_aplha_prod*original_samples)+(sqrt_one_minus_aplha_prod) * noise
