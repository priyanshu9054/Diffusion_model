...
class SelfAttention(nn.Module):
    ...
    def forward(self, x:torch.Tensor, causal_mask = False):
        ...
        if causal_mask:
            # Mask wherre the upper triangle (above the principal diagonal) is made up of 1
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y):
        # x: (latent): (Bstch_Size, Seq_Len_Q, Dim_q)
        # y: (latent): (Bstch_Size, Seq_Len_KV, Dim_KV) = (Batch_Size, 77, 768)
