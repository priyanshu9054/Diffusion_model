...
def generate(prompt: str, 
    ...
    with torch.no_grad():
        ...
        if do_cfg:
            ...
            cond_tokens = tokenizer.bach_encode_plus([prompt], padding="max_length", max_length=77).input_ids
            # (Batch_Size, Seq_len)
            cond_tokens = torch.tensor(cond_tokens, dtype=torch.long, device=device)
            # (Batch_Size, Seq_Len) -> (Batch_Size, Seq_Len, Dim)
            cond_context = clip(cond_tokens)

            uncond_tokens = tokenizer.bach_encode_plus([uncond_prompt], padding="max_length", max_length=77).input_ids
            ...
            tokens = tokenizer.bach_encode_plus([prompt], padding="max_length", max_length=77).input_ids
            # (Batch_Size, Seq_Len)
        ...
        if sampler_name == 'ddpm':
            ...
            sampler.set_interface_steps(n_inference_steps)
        else:
        ...
        for i,timestep in enumerate(timesteps):
            ...
            latents = sampler.steps(timestep, latents, model_output)
...
def rescale(x, old_range, new_range, clamp=False):
    ...
    x -= old_min
    x  += (new_max - new_min) / (old_max - new_min)
    x += new_min
    if clamp:
